    ...
    <xsl:for-each select="/*">
      ...
      <xsl:element name="mdb:MD_Metadata">
        ...
        <xsl:call-template name="add-iso19115-3-namespaces"/>
				<xsl:attribute name="xsi:schemaLocation">http://standards.iso.org/19115/-3/mdb/1.0 http://standards.iso.org/19115/-3/mdb/1.0/mdb.xsd</xsl:attribute>
  ...
  <xsl:template match="gmd:metadataStandardName" priority="5">
    ...
    <mdb:metadataStandard>
      <cit:CI_Citation>
        <cit:title>
					<gco:CharacterString>
           	<xsl:value-of select="concat('ISO19115-1:2014/ISO19115-3:2014-12-25 schemas (converted from ',.,')')"/>
					</gco:CharacterString>
        </cit:title>
        <cit:edition>
					<gco:CharacterString>
          	<xsl:value-of select="concat('2014-12-25 schemas (converted from ',.,' with version ',../gmd:metadataStandardVersion,')')"/>
					</gco:CharacterString>
        </cit:edition>
      </cit:CI_Citation>
    </mdb:metadataStandard>
    ...
    <!-- 
      metadataProfile CI_Citation contains GA info
    -->
    <mdb:metadataProfile>
      <cit:CI_Citation>
        <cit:title>
