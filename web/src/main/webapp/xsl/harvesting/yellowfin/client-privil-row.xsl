...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/root/group">
		...
		<tr id="yellowfin.group.{@id}">
			...
			<td class="padded" align="center">
				<!--button class="content" onclick="harvesting.yellowfin.removeGroupRow('yellowfin.group.{@id}')">
					<xsl:value-of select="/root/strings/remove"/>
				</button-->

				<!-- Yet another fix for IE -->

				<a href="javascript:harvesting.yellowfin.removeGroupRow('yellowfin.group.{@id}')">
					<xsl:value-of select="/root/strings/remove"/>
				</a>		
			</td>
		</tr>
